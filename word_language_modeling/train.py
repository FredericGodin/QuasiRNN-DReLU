...
parser.add_argument("--init_W", type=str, default="lasagne.init.Uniform(0.05)", help="initial parameter values")
parser.add_argument("--init_b", type=str, default="lasagne.init.Uniform(0.05)", help="initial parameter values")
...
parser.add_argument("--rec_num_units", type=int, default=200, help="number of LSTM units")
...
parser.add_argument("--rnn_type", type=str, default="qrnn", help="lstm,qrnn,drelu,delu")
...
parser.add_argument("--num_epochs", type=int, default=70, help="Number of epochs to run")
...
# data
parser.add_argument("--save_file", type=str, default="lm")
parser.add_argument("--save_dir", type=str, default="../models/word_language_modeling/")
...
corresponding_test_perplexity = sys.maxint
...
for epoch in range(paras["num_epochs"]):
    ...
    perplexity_test = networks.calc_perplexity_variable(x_test, y_test, paras, f_eval, padding=mask_test)
    ...
    if perplexity_valid < best_valid_perplexity:
        ...
        corresponding_test_perplexity = perplexity_test
    ...
    with  codecs.open(os.path.join(paras["save_dir"], save_file_settings), "a") as f:
        f.write("Epoch %s, valid perplexity %s, train_perplexity %s, test perplexity %s\n" % (
        epoch, perplexity_valid, perplexity_train, perplexity_test))
...
print(paras)
print("Best Valid Perplexity = %s, with Train Perplexity = %s and Test Perplexity = %s" % (
best_valid_perplexity, corresponding_train_perplexity, corresponding_test_perplexity))
