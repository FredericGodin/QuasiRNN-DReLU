...
def calc_perplexity_variable(x,y,paras,f_eval,padding=None):
    ...
    if padding is not None:
        perplexity = np.exp(np.sum(l_cost) / (np.sum(padding)))
    else:
        perplexity = np.exp(np.sum(l_cost) / len(l_cost))

    return perplexity

def gradient_updates(cost,all_params,paras,learning_rate,update_function=lasagne.updates.sgd):
    ...
    all_grads = T.grad(cost*paras["model_seq_len"], all_params)
    ...
    all_grads, norm = lasagne.updates.total_norm_constraint(
        all_grads, paras["max_grad_norm"], return_norm=True)

    updates = update_function(all_grads, all_params, learning_rate=learning_rate)

    return updates, norm

def calc_cross_ent(net_output, targets,paras):
