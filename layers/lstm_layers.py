...
from lasagne import init, random
...
class LSTMLayer(MergeLayer):
    ...
    def get_output_shape_for(self, input_shapes):
        ...
        if self.only_return_final:
            return (input_shape[0], self.num_units),(input_shape[0], self.num_units)
        # Otherwise, the shape will be (n_batch, n_steps, num_units)
        else:
            return (input_shape[0], input_shape[1], self.num_units),(input_shape[0], input_shape[1], self.num_units)

    def get_output_for(self, inputs, deterministic=True, **kwargs):
        """
        ...
        def step(input_n, cell_previous, hid_previous, *args):
            ...
            return [cell, hid, ingate, forgetgate, outgate]

        def step_masked(input_n,  mask_n, cell_previous, hid_previous, *args):
            cell, hid, ingate, forgetgate, outgate = step(input_n,  cell_previous, hid_previous, *args)
        ...
        if self.debug:
            new_outs = [hid_out,cell_out]

            for i in range(2,len(outs)):
                new_outs.append(outs[i].dimshuffle(1, 0, 2))
